...
export default async function exportCommand(interaction: ChatInputCommandInteraction, currentForm: Form) {
	...
	if (applications.length === 0) {
		await interaction.reply({ content: 'Il n y a aucune application à exporter !', ephemeral: true });
		return;
	}
	...
	interaction.editReply({ content: 'Voici le fichier CSV du formulaire :', files: [`./data/${currentForm.form_channel_id}.csv`]});
}