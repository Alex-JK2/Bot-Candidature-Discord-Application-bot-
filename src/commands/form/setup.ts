...
export default async function setupCommand(interaction: ChatInputCommandInteraction) {
	var channel = interaction.options.getChannel('submit_channel')?.id.toString();
	editFormModal(interaction)
		...
		.catch((err) => {
			console.log(err);
			interaction.followUp({ content: 'La configuration du formulaire a été annulée ou quelque chose s est mal passé !', ephemeral: true });
		});
}