...
export default async function removeCommand(interaction: ChatInputCommandInteraction, currentForm: Form) {
	...
	await response.update({ content: 'La question a été supprimée du formulaire', components: [], embeds: [] });

	const updatedQuestions = await currentForm.$get('question');

	if (updatedQuestions.length < 1) {
		await currentForm.update({ enabled: false });
		await interaction.followUp({ content: "Le questionnaire n'a pas de formulaire ! Les soumissions de formulaires ont été désactivées. Veuillez ajouter une question avant d'activer les soumissions. ", ephemeral: true });
	}
}